...
class PlaceMapState extends State<PlaceMap> {
  ...
  Future<void> onMapCreated(GoogleMapController controller) async {
    ...
    var markers = <Marker>{};
    for (var place in MapAppState.of(context).places) {
    ...
    await _zoomToFitPlaces(
      _getPlacesForCategory(
        MapAppState.of(context).selectedCategory,
  ...
  Future<Marker> _createPlaceMarker(BuildContext context, Place place) async {
    final marker = Marker(
      ...
      visible: place.category == MapAppState.of(context).selectedCategory,
    );
  ...
  void _onPlaceChanged(Place value) {
    ...
    final newPlaces = List<Place>.from(MapAppState.of(context).places);
    ...
    // in the main build method due to a modified MapAppState.
    _configuration = MapConfiguration(
      places: newPlaces,
      selectedCategory: MapAppState.of(context).selectedCategory,
    );

    MapAppState.updateWith(context, places: newPlaces);
  }
  ...
  Future<void> _switchSelectedCategory(PlaceCategory category) async {
    MapAppState.updateWith(context, selectedCategory: category);
  ...
  Future<void> _confirmAddPlace(BuildContext context) async {
    if (_pendingMarker != null) {
      ...
      final newPlace = Place(
        ...
        category: MapAppState.of(context).selectedCategory,
      );

      var placeMarker = await _getPlaceMarkerIcon(
          context, MapAppState.of(context).selectedCategory);
  ...
  Future<void> _maybeUpdateMapConfiguration() async {
    _configuration ??= MapConfiguration.of(MapAppState.of(context));
    final newConfiguration = MapConfiguration.of(MapAppState.of(context));
      ...
      return Center(
        child: Stack(
          children: <Widget>[
            ...
            _CategoryButtonBar(
              selectedPlaceCategory: MapAppState.of(context).selectedCategory,
...
class MapConfiguration {
  ...
  static MapConfiguration of(MapAppState appState) {
